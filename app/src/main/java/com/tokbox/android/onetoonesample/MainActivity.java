...
import static com.tokbox.android.onetoonesample.config.OpenTokConfig.API_KEY;
import static com.tokbox.android.onetoonesample.config.OpenTokConfig.SECRET_KEY;
...
public class MainActivity extends AppCompatActivity implements PreviewControlFragment.PreviewControlCallbacks,
    ...
    /*   OpenTok opentok = new OpenTok(API_KEY, SECRET_KEY);
        ...
        OTConfig config =
                new OTConfig.OTConfigBuilder(OpenTokConfig.SESSION_ID, OpenTokConfig.TOKEN,
                        API_KEY).name("one-to-one-sample-app").subscribeAutomatically(false).subscribeToSelf(false).build();
        if ( config != null ) {
